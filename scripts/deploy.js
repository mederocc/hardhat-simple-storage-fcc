...
async function main() {
  ...
  const WETHFactory = await ethers.getContractFactory('wethQuest');
  console.log('deploying contract');

  const wethStorage = await WETHFactory.deploy();
  ...
  await wethStorage.deployed();
  console.log(`Deployed contract to: ${wethStorage.address}`);
  ...
  if (network.config.chainId === 11155111 && process.env.ETHERSCAN_API_KEY) {
    ...
    await wethStorage.deployTransaction.wait(6);
    await verify(wethStorage.address, []);
  }

  // const currentValue = await wethStorage.deposit();
  // console.log('Current value is: ' + currentValue);
  // const transactionResponde = await wethStorage.store(7);
  // await transactionResponde.wait(1);
  // const updatedValue = await wethStorage.retrieve();
